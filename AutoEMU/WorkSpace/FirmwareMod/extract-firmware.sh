...
#Print statement not useful for my usecase
...
case ${FS_TYPE} in
	"squashfs")
		...
		FS_ARGS=""
		FS_MKFS=$(./unsquashfs_all.sh "${FSIMG}" "${ROOTFS}" 2>/dev/null | grep MKFS) ##
...
if [ ${?} -eq 0 ]; then
	echo "Extraction Success - 0"
	#echo "Firmware parts can be found in '${DIR}/*'"
else
	echo "Extraction Failure - 1"
	rm -rf "${DIR}"
	exit 1
fi
